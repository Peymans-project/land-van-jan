...
const pages = {
  ...
  '/leden': { title: 'Ledenomgeving | Land van Jan Huissen', description: 'Jouw profiel en activiteiten bij Land van Jan.' },
  '/beheer': { title: 'Beheer | Land van Jan Huissen', description: 'Beheer leden en e-mailconcepten voor Land van Jan.' },
};

async function api(path, options = {}) {
  const response = await fetch(path, { credentials: 'include', headers: { 'Content-Type': 'application/json', ...(options.headers || {}) }, ...options });
  const data = await response.json().catch(() => ({}));
  if (!response.ok) throw new Error(data.error || data.message || 'De server kon deze aanvraag niet verwerken.');
  return data;
}
...
function App() {
  ...
  useEffect(() => { api('/api/auth/me').then(({ user }) => setMember(user || null)).catch(() => setMember(null)).finally(() => setAuthReady(true)); }, []);
  useEffect(() => {
  ...
  const authenticateApi = async ({ mode, name, email, password, consent }) => {
    const normalizedEmail = email.trim().toLowerCase();
    const body = mode === 'register' ? { name: name.trim(), email: normalizedEmail, password, marketingConsent: consent } : { email: normalizedEmail, password };
    const result = await api(mode === 'register' ? '/api/auth/register' : '/api/auth/login', { method: 'POST', body: JSON.stringify(body) });
    setMember(result.user); setNotice(`Welkom ${result.user?.name || 'terug'}. Je ledenomgeving is klaar.`);
  };
  ...
  if (path === '/leden' || path === '/beheer') return <main className="dashboard-shell">{header}{!member ? <ProtectedGate openLogin={() => setLoginOpen(true)} /> : path === '/leden' ? <MemberDashboard member={member} /> : <AdminDashboard member={member} />}{loginOpen && <Login close={() => setLoginOpen(false)} authenticate={authenticateApi} member={member} logout={logout} />}{notice && <Notice close={() => setNotice('')}>{notice}</Notice>}</main>;
  if (path !== '/') return <main className="inner-page">{header}<Page path={path} navigate={navigate} setNotice={setNotice} setLoginOpen={setLoginOpen} member={member} />{loginOpen && <Login close={() => setLoginOpen(false)} authenticate={authenticateApi} member={member} logout={logout} />} {notice && <Notice close={() => setNotice('')}>{notice}</Notice>}</main>;
  ...
  return <main>{header}
    ...
    <Footer navigate={navigate} login={() => setLoginOpen(true)} member={member} />{loginOpen && <Login close={() => setLoginOpen(false)} authenticate={authenticateApi} member={member} logout={logout} />}{notice && <Notice close={() => setNotice('')}>{notice}</Notice>}</main>;
}
...
function Page({ path, navigate, setNotice, setLoginOpen, member }) {
  const content = {
  ...
  if (path === '/agenda') return <AgendaPage content={content} member={member} navigate={navigate} openLogin={() => setLoginOpen(true)} />;
...
function AgendaPage({ content, member, openLogin }) { return <><section className="page-hero"><img src={content.image} alt={content.alt} /><div /><div><p className="eyebrow light">{content.eyebrow}</p><h1>{content.title}</h1><p>{content.text}</p></div></section><ActivityList member={member} openLogin={openLogin} /><PageGallery content={content} /><section className="page-quote"><p>“{content.quote}”</p><span>LAND VAN JAN · HUISSEN</span></section></>; }
...
function ActivityList({ member, openLogin }) {
  const [items, setItems] = useState([]); const [status, setStatus] = useState('loading'); const [message, setMessage] = useState('');
  useEffect(() => { api('/api/activities').then(data => { setItems(data.activities || []); setStatus('ready'); }).catch(() => { setItems(activities); setStatus('offline'); }); }, []);
  const register = async id => { if (!member) return openLogin(); setMessage(''); try { const result = await api(`/api/activities/${id}/register`, { method: 'POST' }); setMessage(result.message || 'Je bent aangemeld.'); } catch (err) { setMessage(err.message); } };
  return <section className="programme section"><div className="section-intro"><p className="eyebrow">KOMENDE MOMENTEN</p><h2>Doe mee op<br />jouw manier.</h2></div>{message && <p className="form-status">{message}</p>}<div className="activity-list">{items.map(a => <article className="activity" key={a.id || a.title}><div className="date"><span>{a.day || 'NU'}</span><strong>{a.date || '—'}</strong><span>{a.month || ''}</span></div><p className="time">{a.time || 'Tijd volgt'}</p><div><h3>{a.title}</h3><p>{a.text || a.description}</p></div><button className="text-button" onClick={() => register(a.id)}> {member ? 'Aanmelden' : 'Log in om aan te melden'} <span>→</span></button></article>)}</div>{status === 'offline' && <p className="empty-copy">De live agenda wordt geladen zodra de activiteitenservice actief is.</p>}</section>;
}

function ProtectedGate({ openLogin }) { return <section className="dashboard-gate"><p className="eyebrow">LEDENOMGEVING</p><h1>Je eigen<br /><em>plek op het land.</em></h1><p>Log in om je profiel, updates en activiteiten te bekijken.</p><button className="button" onClick={openLogin}>Inloggen <span>→</span></button></section>; }

function MemberDashboard({ member }) {
  const [profile, setProfile] = useState(null); const [state, setState] = useState('loading'); const [error, setError] = useState('');
  const [registrations, setRegistrations] = useState([]);
  const [billingBusy, setBillingBusy] = useState(false); const [billingError, setBillingError] = useState('');
  useEffect(() => { let active = true; api('/api/member/profile').then(data => { if (active) { setProfile(data.profile || data); setState('ready'); } }).catch(err => { if (active) { setError(err.message); setState('error'); } }); return () => { active = false; }; }, []);
  useEffect(() => { api('/api/member/registrations').then(data => setRegistrations(data.registrations || [])).catch(() => setRegistrations([])); }, []);
  const manageMembership = async () => { setBillingBusy(true); setBillingError(''); try { const endpoint = ['active', 'trialing', 'past_due'].includes(profile?.membershipStatus) ? '/api/billing/portal' : '/api/billing/checkout'; const data = await api(endpoint, { method: 'POST', body: '{}' }); window.location.assign(data.portalUrl || data.checkoutUrl); } catch (err) { setBillingError(err.message); } finally { setBillingBusy(false); } };
  return <section className="dashboard-content"><div className="dashboard-kicker"><p className="eyebrow">LEDENOMGEVING</p><span>● Veilig ingelogd</span></div><h1>Welkom,<br /><em>{member.name || 'lid'}.</em></h1>{state === 'loading' && <DashboardLoading />}{state === 'error' && <DashboardError message={error} />}{state === 'ready' && <><div className="dashboard-grid"><article><p className="eyebrow">JOUW PROFIEL</p><h2>{profile.name || member.name}</h2><p>{profile.email || member.email}</p><button className="text-button">Profiel aanpassen <span>→</span></button></article><article><p className="eyebrow">LIDMAATSCHAP</p><h2>{profile.membershipStatus || 'In afwachting'}</h2><p>Je ontvangt activiteiten en updates zodra je lidmaatschap volledig is ingesteld.</p><button className="text-button" onClick={manageMembership} disabled={billingBusy}>{billingBusy ? 'Even wachten…' : ['active', 'trialing', 'past_due'].includes(profile.membershipStatus) ? 'Beheer lidmaatschap' : 'Start lidmaatschap'} <span>→</span></button>{billingError && <p className="form-status" role="alert">{billingError}</p>}</article><article><p className="eyebrow">NIEUWSBRIEF</p><h2>{profile.marketingConsent ? 'Aangemeld' : 'Niet aangemeld'}</h2><p>Je voorkeuren blijven altijd aanpasbaar.</p></article></div><section className="dashboard-agenda"><div><p className="eyebrow">JOUW AANMELDINGEN</p><h2>Op het land.</h2></div><div>{registrations.length ? registrations.map(item => <article key={item.id}><b>{item.date || 'Binnenkort'}</b><span>{item.activityTitle || item.title}</span><small>{item.status === 'cancelled' ? 'Geannuleerd' : 'Aangemeld'}</small></article>) : <p className="empty-copy">Je hebt nog geen komende aanmeldingen.</p>}</div></section></>}</section>;
}

function LegacyAdminDashboard({ member }) {
  const [state, setState] = useState('loading'); const [members, setMembers] = useState([]); const [error, setError] = useState(''); const [message, setMessage] = useState(''); const [sending, setSending] = useState(false);
  useEffect(() => { let active = true; api('/api/admin/members?limit=50').then(data => { if (active) { setMembers(data.members || []); setState('ready'); } }).catch(err => { if (active) { setError(err.message); setState('error'); } }); return () => { active = false; }; }, []);
  const submitDraft = async event => { event.preventDefault(); const form = new FormData(event.currentTarget); setSending(true); setMessage(''); try { const result = await api('/api/admin/bulk-email', { method: 'POST', body: JSON.stringify({ subject: form.get('subject'), body: form.get('body'), audience: 'consented-members', mode: 'draft' }) }); setMessage(result.message || 'Concept is opgeslagen. Er is niets verstuurd.'); } catch (err) { setMessage(err.message); } finally { setSending(false); } };
  if (!member.isAdmin && member.role !== 'admin') return <section className="dashboard-gate"><p className="eyebrow">BEHEER</p><h1>Geen toegang.</h1><p>Deze omgeving is alleen beschikbaar voor geautoriseerde beheerders.</p></section>;
  return <section className="dashboard-content admin-dashboard"><div className="dashboard-kicker"><p className="eyebrow">BEHEERDERSOMGEVING</p><span>● Privé</span></div><h1>Leden &<br /><em>berichten.</em></h1>{state === 'loading' && <DashboardLoading />}{state === 'error' && <DashboardError message={error} />}{state === 'ready' && <><section className="admin-stats"><article><b>{members.length}</b><span>leden geladen</span></article><article><b>{members.filter(item => item.marketingConsent).length}</b><span>e-mailtoestemming</span></article><article><b>{members.filter(item => item.membershipStatus === 'active').length}</b><span>actieve leden</span></article></section><section className="member-table"><div className="table-title"><h2>Leden</h2><p>Alleen noodzakelijke gegevens, uitsluitend voor beheer.</p></div><div className="table-wrap"><table><thead><tr><th>Naam</th><th>E-mail</th><th>Toestemming</th><th>Status</th></tr></thead><tbody>{members.map(item => <tr key={item.id}><td>{item.name}</td><td>{item.email}</td><td>{item.marketingConsent ? 'Ja' : 'Nee'}</td><td>{item.membershipStatus || 'Nieuw'}</td></tr>)}</tbody></table></div></section><section className="composer"><div><p className="eyebrow light">E-MAILCONCEPT</p><h2>Stuur pas<br /><em>na controle.</em></h2><p>Alleen leden met expliciete e-mailtoestemming worden geselecteerd. Deze knop slaat uitsluitend een concept op.</p></div><form onSubmit={submitDraft}><label>Onderwerp<input name="subject" required placeholder="Bijvoorbeeld: Zaaidag in de kas" /></label><label>Bericht<textarea name="body" required rows="5" placeholder="Schrijf hier je bericht…" /></label>{message && <p className="form-status">{message}</p>}<button className="button" disabled={sending}>{sending ? 'Concept opslaan…' : 'Sla concept op'} <span>→</span></button></form></section></>}</section>;
}

function AdminDashboard({ member }) {
  if (!member.isAdmin && member.role !== 'admin') return <section className="dashboard-gate"><p className="eyebrow">BEHEER</p><h1>Geen toegang.</h1><p>Deze omgeving is alleen beschikbaar voor geautoriseerde beheerders.</p></section>;
  return <><LegacyAdminDashboard member={member} /><ActivityManager /></>;
}

function ActivityManager() {
  const [items, setItems] = useState([]); const [state, setState] = useState('loading'); const [message, setMessage] = useState(''); const [attendees, setAttendees] = useState([]); const [selected, setSelected] = useState('');
  const reload = () => api('/api/admin/activities').then(data => { setItems(data.activities || []); setState('ready'); }).catch(err => { setMessage(err.message); setState('error'); });
  useEffect(() => { reload(); }, []);
  const save = async event => { event.preventDefault(); const form = new FormData(event.currentTarget); const body = { title: form.get('title'), description: form.get('description'), startsAt: form.get('startsAt'), capacity: Number(form.get('capacity')), published: form.get('published') === 'on' }; try { await api('/api/admin/activities', { method: 'POST', body: JSON.stringify(body) }); setMessage('Activiteit opgeslagen.'); event.currentTarget.reset(); reload(); } catch (err) { setMessage(err.message); } };
  const showAttendees = async id => { setSelected(id); setAttendees([]); try { const data = await api(`/api/admin/activities/${id}/registrations`); setAttendees(data.registrations || []); } catch (err) { setMessage(err.message); } };
  return <section className="dashboard-content activity-manager"><div className="dashboard-kicker"><p className="eyebrow">ACTIVITEITENBEHEER</p><span>● Privé</span></div><h1>Agenda<br /><em>maken.</em></h1>{message && <p className="form-status">{message}</p>}<div className="manager-grid"><form className="manager-form" onSubmit={save}><h2>Nieuwe activiteit</h2><label>Titel<input name="title" required placeholder="Bijvoorbeeld: Oogstdag" /></label><label>Datum en tijd<input name="startsAt" type="datetime-local" required /></label><label>Capaciteit<input name="capacity" type="number" min="1" required placeholder="12" /></label><label>Beschrijving<textarea name="description" rows="4" required placeholder="Wat gaan we doen?" /></label><label className="consent"><input name="published" type="checkbox" />Publiceer in agenda</label><button className="button">Opslaan <span>→</span></button></form><div className="manager-list"><h2>Bestaande activiteiten</h2>{state === 'loading' && <DashboardLoading />}{state === 'error' && <DashboardError message={message} />}{state === 'ready' && (items.length ? items.map(item => <article key={item.id}><div><b>{item.title}</b><span>{item.startsAt || 'Datum volgt'} · {item.capacity || 0} plekken · {item.published ? 'Gepubliceerd' : 'Concept'}</span></div><button className="text-button" onClick={() => showAttendees(item.id)}>Deelnemers <span>→</span></button></article>) : <p className="empty-copy">Nog geen activiteiten aangemaakt.</p>)}</div></div>{selected && <section className="attendee-list"><div className="table-title"><h2>Deelnemers</h2><button className="text-button" onClick={() => setSelected('')}>Sluiten</button></div>{attendees.length ? attendees.map(person => <article key={person.id}><b>{person.name}</b><span>{person.email}</span><small>{person.status || 'Aangemeld'}</small></article>) : <p className="empty-copy">Geen deelnemers gevonden.</p>}</section>}</section>;
}

function DashboardLoading() { return <div className="dashboard-state" role="status"><span className="loader" />Laden…</div>; }
function DashboardError({ message }) { return <div className="dashboard-state error"><b>Deze gegevens zijn nu niet beschikbaar.</b><span>{message}</span></div>; }
