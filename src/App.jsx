...
function AdminDashboard({ member }) {
  ...
  return <section className="dashboard-content admin-dashboard">
    ...
    {state === 'ready' && <>
      <section className="admin-stats"><article><b>{members.length}</b><span>leden</span></article><article><b>{activities.filter((item) => item.status === 'published').length}</b><span>gepubliceerde activiteiten</span></article><article><b>{messages.length}</b><span>contactberichten</span></article><article><b>{BILLING_READY_STATES.has(setup?.billing) ? 'Gereed' : 'Actie'}</b><span>betaalconfiguratie</span></article><article><b>{setup?.email === 'configured' ? 'Gereed' : 'Actie'}</b><span>automatische e-mail</span></article></section>
